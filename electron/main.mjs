...
// Tell the scraper where the scraper-host entry lives so it can reuse this
// app's own Electron binary for headless scraping (no separate browser).
process.env.PLANBROWSER_SCRAPER_HOST = path.join(__dirname, 'scraper-host.cjs');
...
async function startServer() {
  ...
  // Listen on an ephemeral port so we never collide with a running dev server
