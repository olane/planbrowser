  ...
  <div class="bg-white p-4 rounded shadow border border-gray-200 min-w-0">
    <div class="flex flex-wrap justify-between items-start gap-x-4 gap-y-1 mb-1">
      <h3 class="font-bold text-lg flex items-center gap-2 min-w-0">
        <button @click="toggleStar" :title="app.starred ? 'Remove from favourites' : 'Add to favourites'" class="cursor-pointer text-gray-400 hover:text-yellow-500 disabled:opacity-50 shrink-0">
        ...
        <router-link :to="`/app/${encodeURIComponent(app.reference)}`" class="hover:underline break-all min-w-0">{{ app.reference }}</router-link>
